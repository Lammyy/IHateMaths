...
with tf.device('/gpu:0'):
    ...
    nelbo=tf.reduce_mean(likeli)+tf.reduce_mean(tf.log(ratio_post))
...
#with tf.Session() as sess:
    ...
    for j in range(50001):
        ...
        if j % 100 == 0:
            ...
            plt.savefig('FiguresMNISTPCKL\Fig %i'%(j+1), bbox_inches='tight')
            plt.close(fig2)
    plt.subplot(2,1,1)
    plt.plot(RECON)
    ...
    plt.savefig('FiguresMNISTPCKL\Diag Plot', bbox_inches='tight')
    plt.close()
