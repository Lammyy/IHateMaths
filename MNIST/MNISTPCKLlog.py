...
#with tf.Session() as sess:
    ...
    for j in range(50001):
        ...
        if j % 100 == 0:
            ...
            fig = plot(samples)
            plt.savefig('FiguresMNISTPCKLlog\Fig %i'%(j), bbox_inches='tight')
            plt.close(fig)
            ...
            plt.savefig('FiguresMNISTPCKLlog\Fig %i'%(j+1), bbox_inches='tight')
            plt.close(fig2)
    plt.subplot(2,1,1)
    plt.plot(RECON)
    ...
    plt.savefig('FiguresMNISTPCKLlog\Diag Plot', bbox_inches='tight')
    plt.close()
