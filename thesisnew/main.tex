...
%We want to estimate $q(u)/p(u)$.
%\vspace{0.3cm}
%  \begin{enumerate}
%  \item {
%    Discriminator network $D_\alpha(u)$ finds probability that a sample $u$ came from $q(u)$.
%  }
%  \[D^*_\alpha(u)=\frac{q(u)}{q(u)+p(u)}\]
%  % or you can use the \uncover command to reveal general
%  % content (not just \items):
%  \item {
%    Train discriminator with Bernoulli loss.
%  }
%  \vspace{0.3cm}
%  \item Express density ratio in terms of discriminator.
