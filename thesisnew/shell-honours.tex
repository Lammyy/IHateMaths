...
\end{equation*}
Note that the KL divergence is not symmetric. We use the reverse KL divergence instead of the forward KL divergence $KL(p(x)||q(x))$ because we are unable to sample from $p(z|x)$, and because it leads to an expectation maximization algorithm as opposed to an expectation propagation algorithm.\\
Using this expression, we can rewrite equation 3.2.1 as:
\begin{align*}
...
\end{align}
The term in line 3.4.2 is a constant as it does not depend on $q_j(z_j)$. Now our Lagrangian equation with the constraint that $q_i(z_i)$ are probability density functions is:
\begin{equation*}
ELBO(q)-\sum^M_{i=1}\lambda_i\int_\mathcal{z_i}q_i(z_i)dz_i=0
\end{equation*}
or using our expression for $ELBO(q)$ in line 3.4.3,
\begin{equation}
...
\end{equation}
\newpage 
We then take the functional derivative of Equation 3.4.4 with respect to $q_j(z_j)$:
\begin{align}
...
\end{align}
Equating expression 3.4.5 to 0 and letting $1+\lambda_j$ be a constant (as it is independent of $z$), we have:
\begin{align}
...
\end{align}
The normalization constant on the denominator of 3.4.6 is derived by observing $q^*_j(z_j)$ as a density. Finally, we derive a simpler expression of $q^*_j(z_j)$ by observing that terms independent of $z_j$ can be treated as a constant:
\begin{align}
...
\end{align*}
Using this and equation 3.5.1, we can derive the evidence lower bound as a function of the variational parameters:
\begin{align*}
...
\end{align*}
From equation 3.4.7, we derive the optimal categorical factor by only considering terms from the true distribution $p(.)$ dependent on $c_i$:
...
We can now evaluate the second term of equation 3.5.2:
\begin{align*}
...
\newpage
Now we find the variational density of the $k$th mixture component, again using equation 3.4.7 with the ELBO and ignoring terms independent of $p(.)$ and $\mu_k$:
...
\[q_\phi(z|x)\sim \mathcal{N}(\mu,\sigma^2).\]
...
\[\epsilon\sim \mathcal{N}(0,I),\qquad z=\mu+\epsilon\cdot\sigma^2.\]
