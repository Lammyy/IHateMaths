...
\[KL(q_\phi(\bm{z}|\bm{x})\|p(\bm{z}))=\frac12 \sum_{i=1}^k\left(\sigma^2_i+\mu^2_i-\log(\sigma^2_i)-1\right).\]
\begin{proof}
...
\[q_\phi(\bm{z}|\bm{x})=\frac{1}{(2\pi)^{k/2}\det(\Sigma)^{1/2}}\exp\left(-\frac{1}{2}(\bm{z}-\bm{\mu})^\top\Sigma^{-1}(\bm{z}-\bm{\mu})\right),\]
\[p(\bm{z})=\frac{1}{(2\pi)^{k/2}}\exp\left(-\frac{1}{2}\bm{z}^\top I\bm{z}\right).\]
...
&=-\frac{1}{2}\log(\det \Sigma)-\frac{k}{2}+\frac{1}{2}\tr\left(I+\left(\Sigma+\mu\mu^\top\right)\right)\\
&=\frac12 \sum_{i=1}^k\left(\sigma^2_i+\mu^2_i-\log(\sigma^2_i)-1\right).
\end{align*}
\end{proof}
\end{lemma}
