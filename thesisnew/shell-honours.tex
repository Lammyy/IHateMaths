...
{\bigskip}By far my greatest thanks must go to my supervisors Scott Sisson and Edwin Bonilla for the guidance, care and support they provided throughout this year. I would also like to thank Edwin's PhD student Louis Tiao, for guiding me through some crucial areas in this thesis.

{\bigskip\noindent}My second greatest thanks goes to my senior high school mathematics teacher Trevor Trotman. His passion and excellent teaching was crucial to helping my discover my enjoyment of mathematics. If not for him, I would have likely taken a different path in life.

{\bigskip\noindent}Honours Room Inhabitants

{\bigskip\noindent}Shout-out to the Discord fellas for entertaining me and helping me pursue a career as a professional DJ. Extra shout-out to Davey cause he asked for one.

{\bigskip\noindent}Also shout-out to Kong for letting me leech off his gym pass, so that I wouldn't lose too many gains over the thesis period.

{\bigskip\noindent}I would also like to thank Matthew for setting up my github, Sunchit for his Python tech support, and Allan for showing me that the Euler-Lagrange equation can be employed to find the second derivatives of my loss functionals.

{\bigskip\noindent}누나

{\bigskip\noindent}Lastly, I would like to thank Jin, Eliza and Brian for showing me how to make the most out of my life.
...
\KwIn{Dataset density $q^*(x)$, (implicit) prior $p(z)$, likelihood $p(x|z)$, noise density $\pi(\epsilon)$}
...
\KwIn{Dataset density $q^*(x)$, (implicit) prior $p(z)$, (implicit) likelihood $p(x|z)$, noise density $\pi(\epsilon)$}
\KwResult{Optimized posterior generator $\mathcal{G}_\phi(\epsilon;x)$}
\BlankLine
\For{$j=1$ \KwTo $J$}{
	\For{$k=1$ \KwTo $K$}{	
...
The structure of the estimator (discriminator $D_\alpha(\bm{z},x)$, direct ratio estimator $r_\alpha(\bm{z},x)$ or direct log ratio estimator $T_\alpha(\bm{z},x)$) is similar to that of the generator, with an additional hidden layer associated with the $\bm{z}$ input and a different output activation function corresponding to the estimator's identity. When the estimator takes the parametrisation of a discriminator $D_\alpha(z,x)$, a sigmoid output layer is used \citep{gan}. An exponential activation function $g(x)={\rm e}^x$ is used for the output layer when the estimator output is the direct density ratio $r_\alpha(z,x)$, and a linear output is used for the direct log density ratio estimator $T_\alpha(z,x)$ \citep{nowozin}. The estimator structure is shown in \autoref{fig:6.3}.
\begin{figure}[h!]
...
\small Note that in the following plots, there is no noticeable difference between the estimator parametrisations or the $f$-divergences.
\begin{figure}[h]
\centering
