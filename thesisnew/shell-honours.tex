...
\end{lemma}
We use the reverse KL divergence instead of the forward KL divergence $KL(p(x)||q(x))$ because we are unable to sample from our true posterior $p(z|x)$, and because it leads to an expectation maximization algorithm as opposed to an expectation propagation algorithm.
...
\end{align}
The term in line 3.5.2 is a constant as it does not depend on $q_j(z_j)$. Now our Lagrangian equation with the constraint that $q_i(z_i)$ are probability density functions is:
\begin{equation*}
...
\end{align}
Equating expression 3.5.5 to 0 and letting $1+\lambda_j$ be a constant (as it is independent of $z$), we have:
\begin{align}
...
\end{align*}
We take the negative to form the minimization problem:
...
However, if either density is implicit, then density ratio estimation methods must be applied to calculate $KL(q(z|x)||p(z))$.\\
