...
def discriminator(z, x):
    ...
    out_layer = tf.exp(tf.matmul(hidden_layer32, weights['disc_out'])+biases['disc_out'])
    return out_layer
#Build Networks
...
with tf.device('/gpu:0'):
    ...
    nelbo=-tf.reduce_mean(likelihood(post_sample, x_input))+tf.reduce_mean(tf.log(disc_post))
...
#with tf.Session() as sess:
    ...
    for j in range(2001):
        ...
        if j % 100 == 0:
            ...
            # plt.close()
    np.savetxt("PCADVexp truklavg.csv", KLAVGBRUH, delimiter=",")
    np.savetxt("PCADVexp estimatorloss.csv", ISTHISLOSS, delimiter=",")
    np.savetxt("PCADVexp nelbo.csv", NELBRUH, delimiter=",")
    np.savetxt("PCADVexp converge.csv", converge)
    plt.plot(KLAVGBRUH)
    plt.axvline(x=(converge[0]/100), ls='--')
    plt.xlabel('Iterations (x100)')
    plt.ylabel('Avg KL Div')
    plt.savefig("PCADVexp truklavg")
    plt.close()
    plt.plot(ISTHISLOSS)
    plt.axvline(x=converge[0], ls='--')
    plt.xlabel('Iterations')
    plt.ylabel('Estimator Loss')
    plt.savefig("PCADVexp estimatorloss")
    plt.close()
    plt.plot(NELBRUH)
    plt.axvline(x=converge[0], ls='--')
    plt.xlabel('Iterations')
    plt.ylabel('NELBO')
    plt.savefig('PCADVexp nelbo')
    plt.close()
    #nice plot
    sns.set_style('whitegrid')
    sns.set_context('poster')
    ...
    for i in range(5):
       ...
       plt.xticks([])
       plt.yticks([])
    plt.savefig('OCADVexp Final Fig')
    plt.close()
