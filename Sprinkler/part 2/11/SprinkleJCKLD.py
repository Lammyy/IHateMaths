...
with tf.device('/gpu:0'):
    ...
    ratio_loss = -tf.reduce_mean(tf.log((ratio_post+epsilon)/(1-ratio_post+epsilon)))+tf.reduce_mean((ratio_prior+epsilon)/(1-ratio_prior+epsilon))

    nelbo=tf.reduce_mean(tf.log(tf.divide(ratio_post+epsilon,1-ratio_post+epsilon)))
...
#with tf.Session() as sess:
    ...
    np.savetxt("JCKLD truklavg.csv", KLAVGBRUH, delimiter=",")
    np.savetxt("JCKLD estimatorloss.csv", ISTHISLOSS, delimiter=",")
    np.savetxt("JCKLD nelbo.csv", NELBRUH, delimiter=",")
    np.savetxt("JCKLD converge.csv", converge)
    plt.plot(KLAVGBRUH)
    plt.axvline(x=(converge[0]/100), ls='--')
    plt.xlabel('Iterations (x100)')
    plt.ylabel('Avg KL Div')
    plt.savefig("JCKLD truklavg")
    plt.close()
    plt.plot(ISTHISLOSS)
    plt.axvline(x=converge[0], ls='--')
    plt.xlabel('Iterations')
    plt.ylabel('Estimator Loss')
    plt.savefig("JCKLD estimatorloss")
    plt.close()
    plt.plot(NELBRUH)
    plt.axvline(x=converge[0], ls='--')
    plt.xlabel('Iterations')
    plt.ylabel('NELBO')
    plt.savefig('JCKLD nelbo')
    plt.close()
    #nice plot
