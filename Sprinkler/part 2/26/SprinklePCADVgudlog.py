...
with tf.device('/gpu:0'):
    ...
    ratio_loss = tf.reduce_mean(tf.log(tf.exp(ratio_post)+1)-ratio_post)+tf.reduce_mean(tf.log(tf.exp(ratio_prior)+1))

    nelbo=-tf.reduce_mean(likelihood(post_sample, x_input))+tf.reduce_mean(ratio_post)
...
#with tf.Session() as sess:
    ...
    for j in range(2001):
        ...
        if j % 100 == 0:
            ...
            if KLAVG < 1.36 and converge[0]==0:
                converge[0]=int(j)
            ...
            # plt.close()
    np.savetxt("PCADVgudlog truklavg.csv", KLAVGBRUH, delimiter=",")
    np.savetxt("PCADVgudlog estimatorloss.csv", ISTHISLOSS, delimiter=",")
    np.savetxt("PCADVgudlog nelbo.csv", NELBRUH, delimiter=",")
    np.savetxt("PCADVgudlog converge.csv", converge)
    plt.plot(KLAVGBRUH)
    plt.axvline(x=(converge[0]/100), ls='--')
    plt.xlabel('Iterations (x100)')
    plt.ylabel('Avg KL Div')
    plt.savefig("PCADVgudlog truklavg")
    plt.close()
    plt.plot(ISTHISLOSS)
    plt.axvline(x=converge[0], ls='--')
    plt.xlabel('Iterations')
    plt.ylabel('Estimator Loss')
    plt.savefig("PCADVgudlog estimatorloss")
    plt.close()
    plt.plot(NELBRUH)
    plt.axvline(x=converge[0], ls='--')
    plt.xlabel('Iterations')
    plt.ylabel('NELBO')
    plt.savefig('PCADVgudlog nelbo')
    plt.close()
    #nice plot
    sns.set_style('whitegrid')
    sns.set_context('poster')
    ...
    for i in range(5):
       ...
       plt.xticks([])
       plt.yticks([])
    plt.savefig('PCADVgudlog Final Fig')
    plt.close()
