...
#with tf.Session() as sess:
    ...
    np.savetxt("JCADV truklavg.csv", KLAVGBRUH, delimiter=",")
    np.savetxt("JCADV estimatorloss.csv", ISTHISLOSS, delimiter=",")
    np.savetxt("JCADV nelbo.csv", NELBRUH, delimiter=",")
    np.savetxt("JCADV converge.csv", converge)
    plt.plot(KLAVGBRUH)
    plt.axvline(x=(converge[0]/100), ls='--')
    plt.xlabel('Iterations (x100)')
    plt.ylabel('Avg KL Div')
    plt.savefig("JCADV truklavg")
    plt.close()
    plt.plot(ISTHISLOSS)
    plt.axvline(x=converge[0], ls='--')
    plt.xlabel('Iterations')
    plt.ylabel('Estimator Loss')
    plt.savefig("JCADV estimatorloss")
    plt.close()
    plt.plot(NELBRUH)
    plt.axvline(x=converge[0], ls='--')
    plt.xlabel('Iterations')
    plt.ylabel('NELBO')
    plt.savefig('JCADV nelbo')
    plt.close()
    #nice plot
