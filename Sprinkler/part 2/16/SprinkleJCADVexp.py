...
with tf.device('/gpu:0'):
    ...
    disc_loss = -tf.reduce_mean(tf.log(tf.divide(disc_post+epsilon, (disc_post+1+epsilon))))+tf.reduce_mean(tf.log(disc_prior+1+epsilon))
...
#with tf.Session() as sess
    ...
    np.savetxt("JCADVexp truklavg.csv", KLAVGBRUH, delimiter=",")
    np.savetxt("JCADVexp estimatorloss.csv", ISTHISLOSS, delimiter=",")
    np.savetxt("JCADVexp nelbo.csv", NELBRUH, delimiter=",")
    np.savetxt("JCADVexp converge.csv", converge)
    plt.plot(KLAVGBRUH)
    plt.axvline(x=(converge[0]/100), ls='--')
    plt.xlabel('Iterations (x100)')
    plt.ylabel('Avg KL Div')
    plt.savefig("JCADVexp truklavg")
    plt.close()
    plt.plot(ISTHISLOSS)
    plt.axvline(x=converge[0], ls='--')
    plt.xlabel('Iterations')
    plt.ylabel('Estimator Loss')
    plt.savefig("JCADVexp estimatorloss")
    plt.close()
    plt.plot(NELBRUH)
    plt.axvline(x=converge[0], ls='--')
    plt.xlabel('Iterations')
    plt.ylabel('NELBO')
    plt.savefig('JCADVexp nelbo')
    plt.close()
    #nice plot
