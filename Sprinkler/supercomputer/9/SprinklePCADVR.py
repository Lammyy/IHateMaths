...
#with tf.Session() as sess:
    ...
    for j in range(10001):
        ...
        if j % 100 == 0:
            ...
            # plt.close()
    np.savetxt("PCADVR truklavg.csv", KLAVGBRUH, delimiter=",")
    np.savetxt("PCADVR estimatorloss.csv", ISTHISLOSS, delimiter=",")
    np.savetxt("PCADVR nelbo.csv", NELBRUH, delimiter=",")
    np.savetxt("PCADVR converge.csv", converge)
    plt.plot(KLAVGBRUH)
    plt.axvline(x=(converge[0]/100))
    plt.xlabel('Iterations (x100)')
    plt.ylabel('Avg KL Div')
    plt.savefig("PCADVR truklavg")
    plt.close()
    plt.plot(ISTHISLOSS)
    plt.axvline(x=converge[0])
    plt.xlabel('Iterations')
    plt.ylabel('Estimator Loss')
    plt.savefig("PCADVR estimatorloss")
    plt.close()
    plt.plot(NELBRUH)
    plt.axvline(x=converge[0])
    plt.xlabel('Iterations')
    plt.ylabel('NELBO')
    plt.savefig('PCADVR nelbo')
    plt.close()
    #nice plot
    sns.set_style('whitegrid')
    sns.set_context('poster')
