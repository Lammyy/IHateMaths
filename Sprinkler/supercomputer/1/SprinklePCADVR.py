...
#with tf.Session() as sess:
    ...
    for j in range(10001):
        ...
        if j % 100 == 0:
            ...
            # plt.close()
    np.savetxt("1/PCADVR truklavg.csv", KLAVGBRUH, delimiter=",")
    np.savetxt("1/PCADVR estimatorloss.csv", ISTHISLOSS, delimiter=",")
    np.savetxt("1/PCADVR nelbo.csv", NELBRUH, delimiter=",")
    np.savetxt("1/PCADVR converge.csv", converge)
    plt.plot(KLAVGBRUH)
    plt.axvline(x=(converge[0]/100))
    plt.xlabel('Iterations (x100)')
    plt.ylabel('Avg KL Div')
    plt.savefig("1/PCADVR truklavg")
    plt.close()
    plt.plot(ISTHISLOSS)
    plt.axvline(x=converge[0])
    plt.xlabel('Iterations')
    plt.ylabel('Estimator Loss')
    plt.savefig("1/PCADVR estimatorloss")
    plt.close()
    plt.plot(NELBRUH)
    plt.axvline(x=converge[0])
    plt.xlabel('Iterations')
    plt.ylabel('NELBO')
    plt.savefig('1/PCADVR nelbo')
    plt.close()
    #nice plot
    sns.set_style('whitegrid')
    sns.set_context('poster')
