...
with tf.device('/gpu:0'):
    ...
    nelbo=-tf.reduce_mean(likelihood(post_sample, x_input))+tf.reduce_mean(tf.log(tf.divide(ratio_post+epsilon,1-ratio_post+epsilon)))
...
#with tf.Session() as sess:
    ...
    for j in range(10001):
        ...
        if j % 100 == 0:
            ...
            # plt.close()
    np.savetxt("1/PCKLD truklavg.csv", KLAVGBRUH, delimiter=",")
    np.savetxt("1/PCKLD estimatorloss.csv", ISTHISLOSS, delimiter=",")
    np.savetxt("1/PCKLD nelbo.csv", NELBRUH, delimiter=",")
    np.savetxt("1/PCKLD converge.csv", converge)
    plt.plot(KLAVGBRUH)
    plt.axvline(x=(converge[0]/100))
    plt.xlabel('Iterations (x100)')
    plt.ylabel('Avg KL Div')
    plt.savefig("1/PCKLD truklavg")
    plt.close()
    plt.plot(ISTHISLOSS)
    plt.axvline(x=converge[0])
    plt.xlabel('Iterations')
    plt.ylabel('Estimator Loss')
    plt.savefig("1/PCKLD estimatorloss")
    plt.close()
    plt.plot(NELBRUH)
    plt.axvline(x=converge[0])
    plt.xlabel('Iterations')
    plt.ylabel('NELBO')
    plt.savefig('1/PCKLD nelbo')
    plt.close()
    #nice plot
    sns.set_style('whitegrid')
    sns.set_context('poster')
