...
def discriminator(z, x):
    ...
    notsig = tf.matmul(hidden_layer32, weights['disc_out'])+biases['disc_out']
    out_layer = tf.nn.sigmoid(notsig)
    return out_layer
#Build Networks
...
with tf.device('/gpu:0'):
    ...
    nelbo=-tf.reduce_mean(likelihood(post_sample, x_input))+tf.reduce_mean(tf.log(tf.divide(disc_post+epsilon,1-disc_post+epsilon)))
...
#with tf.Session() as sess:
    ...
    for j in range(10001):
        ...
        if j % 100 == 0:
            ...
            if KLAVG < 1.35 and converge[0]==None:
                converge[0]=int(j)
            ...
            #plt.close()
    np.savetxt("1/PCADV truklavg.csv", KLAVGBRUH, delimiter=",")
    np.savetxt("1/PCADV estimatorloss.csv", ISTHISLOSS, delimiter=",")
    np.savetxt("1/PCADV nelbo.csv", NELBRUH, delimiter=",")
    np.savetxt("1/PCADV converge.csv", converge)
    plt.plot(KLAVGBRUH)
    plt.axvline(x=(converge[0]/100))
    plt.xlabel('Iterations (x100)')
    plt.ylabel('Avg KL Div')
    plt.savefig("1/PCADV truklavg")
    plt.close()
    plt.plot(ISTHISLOSS)
    plt.axvline(x=converge[0])
    plt.xlabel('Iterations')
    plt.ylabel('Estimator Loss')
    plt.savefig("1/PCADV estimatorloss")
    plt.close()
    plt.plot(NELBRUH)
    plt.axvline(x=converge[0])
    plt.xlabel('Iterations')
    plt.ylabel('NELBO')
    plt.savefig("1/PCADV nelbo")
    plt.close()
    #nice plot
    sns.set_style('whitegrid')
    sns.set_context('poster')
