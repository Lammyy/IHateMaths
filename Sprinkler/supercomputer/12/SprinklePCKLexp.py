...
with tf.device('/gpu:0'):
    ...
    ratio_loss = -tf.reduce_mean(tf.log(ratio_post))+tf.reduce_mean(ratio_prior)

    nelbo=-tf.reduce_mean(likelihood(post_sample, x_input))+tf.reduce_mean(tf.log(ratio_post))
...
#with tf.Session() as sess:
    ...
    for j in range(10001):
        ...
        if j % 100 == 0:
            ...
            # plt.close()
    np.savetxt("PCKLexp truklavg.csv", KLAVGBRUH, delimiter=",")
    np.savetxt("PCKLexp estimatorloss.csv", ISTHISLOSS, delimiter=",")
    np.savetxt("PCKLexp nelbo.csv", NELBRUH, delimiter=",")
    np.savetxt("PCKLexp converge.csv", converge)
    plt.plot(KLAVGBRUH)
    plt.axvline(x=(converge[0]/100), ls='--')
    plt.xlabel('Iterations (x100)')
    plt.ylabel('Avg KL Div')
    plt.savefig("PCKLexp truklavg")
    plt.close()
    plt.plot(ISTHISLOSS)
    plt.axvline(x=converge[0], ls='--')
    plt.xlabel('Iterations')
    plt.ylabel('Estimator Loss')
    plt.savefig("PCKLexp estimatorloss")
    plt.close()
    plt.plot(NELBRUH)
    plt.axvline(x=converge[0], ls='--')
    plt.xlabel('Iterations')
    plt.ylabel('NELBO')
    plt.savefig('PCKLexp nelbo')
    plt.close()
    #nice plot
    sns.set_style('whitegrid')
    sns.set_context('poster')
