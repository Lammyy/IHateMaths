...
#with tf.Session() as sess:
    ...
    for j in range(10001):
        ...
        if j % 100 == 0:
            ...
            # plt.close()
    np.savetxt("PCADVlog truklavg.csv", KLAVGBRUH, delimiter=",")
    np.savetxt("PCADVlog estimatorloss.csv", ISTHISLOSS, delimiter=",")
    np.savetxt("PCADVlog nelbo.csv", NELBRUH, delimiter=",")
    np.savetxt("PCADVlog converge.csv", converge)
    plt.plot(KLAVGBRUH)
    plt.axvline(x=(converge[0]/100), ls='--')
    plt.xlabel('Iterations (x100)')
    plt.ylabel('Avg KL Div')
    plt.savefig("PCADVlog truklavg")
    plt.close()
    plt.plot(ISTHISLOSS)
    plt.axvline(x=converge[0], ls='--')
    plt.xlabel('Iterations')
    plt.ylabel('Estimator Loss')
    plt.savefig("PCADVlog estimatorloss")
    plt.close()
    plt.plot(NELBRUH)
    plt.axvline(x=converge[0], ls='--')
    plt.xlabel('Iterations')
    plt.ylabel('NELBO')
    plt.savefig('PCADVlog nelbo')
    plt.close()
    #nice plot
    sns.set_style('whitegrid')
    sns.set_context('poster')
    ...
    for i in range(5):
       ...
       plt.xticks([])
       plt.yticks([])
    plt.savefig('PCADVlog Final Fig')
    plt.close()
