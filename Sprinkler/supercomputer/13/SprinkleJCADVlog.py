...
#with tf.Session() as sess:
    ...
    np.savetxt("JCADVlog truklavg.csv", KLAVGBRUH, delimiter=",")
    np.savetxt("JCADVlog estimatorloss.csv", ISTHISLOSS, delimiter=",")
    np.savetxt("JCADVlog nelbo.csv", NELBRUH, delimiter=",")
    np.savetxt("JCADVlog converge.csv", converge)
    plt.plot(KLAVGBRUH)
    plt.axvline(x=(converge[0]/100), ls='--')
    plt.xlabel('Iterations (x100)')
    plt.ylabel('Avg KL Div')
    plt.savefig("JCADVlog truklavg")
    plt.close()
    plt.plot(ISTHISLOSS)
    plt.axvline(x=converge[0], ls='--')
    plt.xlabel('Iterations')
    plt.ylabel('Estimator Loss')
    plt.savefig("JCADVlog estimatorloss")
    plt.close()
    plt.plot(NELBRUH)
    plt.axvline(x=converge[0], ls='--')
    plt.xlabel('Iterations')
    plt.ylabel('NELBO')
    plt.savefig('JCADVlog nelbo')
    plt.close()
    #nice plot
    ...
    for i in range(5):
       ...
       plt.xticks([])
       plt.yticks([])
    plt.savefig('JCADVlog Final Fig')
    plt.close()
