...
#with tf.Session() as sess
    ...
    np.savetxt("JCADVR truklavg.csv", KLAVGBRUH, delimiter=",")
    np.savetxt("JCADVR estimatorloss.csv", ISTHISLOSS, delimiter=",")
    np.savetxt("JCADVR nelbo.csv", NELBRUH, delimiter=",")
    np.savetxt("JCADVR converge.csv", converge)
    plt.plot(KLAVGBRUH)
    plt.axvline(x=(converge[0]/100), ls='--')
    plt.xlabel('Iterations (x100)')
    plt.ylabel('Avg KL Div')
    plt.savefig("JCADVR truklavg")
    plt.close()
    plt.plot(ISTHISLOSS)
    plt.axvline(x=converge[0], ls='--')
    plt.xlabel('Iterations')
    plt.ylabel('Estimator Loss')
    plt.savefig("JCADVR estimatorloss")
    plt.close()
    plt.plot(NELBRUH)
    plt.axvline(x=converge[0], ls='--')
    plt.xlabel('Iterations')
    plt.ylabel('NELBO')
    plt.savefig('JCADVR nelbo')
    plt.close()
    #nice plot
