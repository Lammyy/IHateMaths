...
#with tf.Session() as sess:
    ...
    np.savetxt("JCKL truklavg.csv", KLAVGBRUH, delimiter=",")
    np.savetxt("JCKL estimatorloss.csv", ISTHISLOSS, delimiter=",")
    np.savetxt("JCKL nelbo.csv", NELBRUH, delimiter=",")
    np.savetxt("JCKL converge.csv", converge)
    plt.plot(KLAVGBRUH)
    plt.axvline(x=(converge[0]/100), ls='--')
    plt.xlabel('Iterations (x100)')
    plt.ylabel('Avg KL Div')
    plt.savefig("JCKL truklavg")
    plt.close()
    plt.plot(ISTHISLOSS)
    plt.axvline(x=converge[0], ls='--')
    plt.xlabel('Iterations')
    plt.ylabel('Estimator Loss')
    plt.savefig("JCKL estimatorloss")
    plt.close()
    plt.plot(NELBRUH)
    plt.axvline(x=converge[0], ls='--')
    plt.xlabel('Iterations')
    plt.ylabel('NELBO')
    plt.savefig('JCKL nelbo')
    plt.close()
    #nice plot
