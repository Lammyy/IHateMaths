...
#with tf.Session() as sess:
    ...
    for j in range(10001):
        ...
        if j % 100 == 0:
            ...
            # plt.close()
    np.savetxt("PCKLlog truklavg.csv", KLAVGBRUH, delimiter=",")
    np.savetxt("PCKLlog estimatorloss.csv", ISTHISLOSS, delimiter=",")
    np.savetxt("PCKLlog nelbo.csv", NELBRUH, delimiter=",")
    np.savetxt("PCKLlog converge.csv", converge)
    plt.plot(KLAVGBRUH)
    plt.axvline(x=(converge[0]/100), ls='--')
    plt.xlabel('Iterations (x100)')
    plt.ylabel('Avg KL Div')
    plt.savefig("PCKLlog truklavg")
    plt.close()
    plt.plot(ISTHISLOSS)
    plt.axvline(x=converge[0], ls='--')
    plt.xlabel('Iterations')
    plt.ylabel('Estimator Loss')
    plt.savefig("PCKLlog estimatorloss")
    plt.close()
    plt.plot(NELBRUH)
    plt.axvline(x=converge[0], ls='--')
    plt.xlabel('Iterations')
    plt.ylabel('NELBO')
    plt.savefig('PCKLlog nelbo')
    plt.close()
    #nice plot
    sns.set_style('whitegrid')
    sns.set_context('poster')
