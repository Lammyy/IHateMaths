...
#with tf.Session() as sess:
    ...
    for j in range(50001):
        ...
        if j % 100 == 0:
            ...
            for i in range(5):
                ...
                plt.xticks([])
                plt.yticks([]);
            plt.text(-60,20,'KLlog Loss: %f, NELBO: %f, KL0: %f, KL1: %f, KL2: %f, KL3: %f, KL4: %f' % (dl, NELBO, KL0, KL1, KL2, KL3, KL4))
    ...
    plt.plot(ISTHISLOSS)
    plt.xlabel('Iterations (x100)')
    plt.ylabel('KLlog Loss')
    plt.subplots_adjust(top=0.95,bottom=0.15,right=0.95,hspace=0.4)
    plt.savefig('FiguresJCKLlog\Diag Plot', bbox_inches='tight')
    plt.close()
